...
Claude should generate code similar to what's in `examples/nextjs-auth/middleware.ts`, but generated fresh from current documentation.

## Step 5: Review the Generated Middleware

Compare what Claude generated to the reference implementation:

```bash
diff <(claude output) examples/nextjs-auth/middleware.ts
```
...
```bash
cat examples/supabase-realtime/realtime-chat.ts
```
...
### Want to Verify Context7 is Running?

Look at Claude Code's logs. You should see something like:

```
[MCP Server] Starting context7...
[MCP Server] context7 initialized successfully
```

If you don't see this, restart Claude Code.
